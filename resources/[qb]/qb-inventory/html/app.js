const InventoryContainer = Vue.createApp({
    ...
    methods: {
        ...
        createGhostElement(slotElement) {
            ...
            const computed = getComputedStyle(slotElement);
            ghostElement.classList.add("drag-ghost");
            ghostElement.style.position = "absolute";
            ghostElement.style.pointerEvents = "none";
            ghostElement.style.opacity = "0.85";
            ghostElement.style.zIndex = "1000";
            // Force exact pixel dimensions so % rules with !important don't override
            ghostElement.style.setProperty("width", computed.width, "important");
            ghostElement.style.setProperty("height", computed.height, "important");
            ghostElement.style.boxSizing = "border-box";
            // Avoid clipping inner contents while dragging
            ghostElement.style.setProperty("overflow", "visible", "important");
            return ghostElement;
        },
        drag(event) {
