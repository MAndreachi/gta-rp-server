...
RegisterNetEvent('qb-inventory:client:updateInventory', function(itemsArg)
    local items = {}
    if itemsArg and type(itemsArg) == "table" then
        items = itemsArg
    else
...
RegisterNUICallback('CloseInventory', function(data, cb)
	...
	if not invName and CurrentDrop then
		invName = CurrentDrop
        CurrentDrop = nil
    end
	-- Always notify server to clear inv_busy; use a safe fallback for player inventory
	TriggerServerEvent('qb-inventory:server:closeInventory', invName or 'player')
    cb('ok')
end)
